...
n_mels_bank = 20                               # Number of Mel Filters in mel-spectrogram
...
for count, audio in enumerate(eval_set_list):
    ...
    mel = librosa.feature.melspectrogram(y=data, sr=fs, n_mels=n_mels_bank, hop_length=int(win_length/2), win_length=win_length, n_fft=win_length)
    mel_list.append(mel)
    records.append(audio)
mel_temp = []
max_len = max(np.shape(x)[1] for x in mel_list)
for item in mel_list:
    ...
    mel_temp.append(temp)
    del temp
mel_list = mel_temp
del mel_temp

for item in mel_list:
    np.reshape(item, (n_mels_bank, max_len, 1))

eval_set_base["Record"] = records
eval_set_base["Mel-Spectrogram"] = mel_list
os.chdir(root_dir)
...
audio_train_database["Mel-Spectrogram"] = mel_list
...
model = Sequential()
model.add(Conv2D(32, (6, 6), padding="valid", activation="relu", input_shape=(n_mels_bank, max_len, 1)))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Conv2D(16, (3, 3), padding="valid", strides=2, activation="relu", input_shape=(n_mels_bank, max_len, 1)))
model.add(MaxPooling2D(pool_size=(2, 2)))
